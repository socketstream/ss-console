...
module.exports = function (socketStream) {
  ...
  socketStream.events.on('server:start', function(serverInstance) {
    ...
    ss.log.info("i".green, "Console Server running on port " + port);
    ...
    var server = net.createServer(function(socket) {
      ...
      socket.on('end', function() {
        ss.log.info("←".green, ("Session ID " + sessionID + " - Console client has disconnected").grey);
      });

      // Handle ECONNRESET
      socket.on('error', function(e) {
        ss.log.error("!".red, ("Socket Error " + e).yellow);
      });
      ...
      ss.log.info("→".cyan, ("Session ID " + sessionID + " - Console client has connected").grey);
